...
def create_inputs(paths, params):
    """
    Creates the input file for the NAME air history run, and return its path.
    """
    
    run_start_time, run_stop_time, time_after_first_hour = \
        get_times(params['RunStart'], params['Duration'], direction='Backward')
    ...
    data = {
        ...
        'ReceptorLoc_X': params['ArrivalLoc_X'],
        'ReceptorLoc_Y': params['ArrivalLoc_Y'],
...
def do_example():
    ...
    input_params = {
        ...
        'HGrid_Ymax': 70.,
        'HGrid_Ymin': 30.,
        #'JobTitle': 'NAME air history run',
        'MainTGrid_dT': 6,
        'ArrivalBottom': 0.,
        'ArrivalLoc_Name': 'MACE_HEAD',
        'ArrivalLoc_X': -9.9,
        'ArrivalLoc_Y': 53.3167,
        'ArrivalStart': datetime.datetime(2018, 1, 1, 0, 0, 0),
        'ArrivalStop': datetime.datetime(2017, 12, 31, 21, 0, 0),
        'ArrivalTop': 10.,
        'RunName': 'Testing of a NAME air history run',
        'RunStart': datetime.datetime(2018, 1, 1, 0, 0, 0),
        'ZGrid': [0., 100., 500., 2000., 5000.],
    }
